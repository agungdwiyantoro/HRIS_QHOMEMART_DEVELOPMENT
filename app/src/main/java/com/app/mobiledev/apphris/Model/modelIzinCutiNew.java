package com.app.mobiledev.apphris.Model;

public class modelIzinCutiNew{
    private String ctgantihari;
    private String approveHrd;
    private String hrdName;
    private String nmcuti;
    private String dvnama;
    private String ctsisahak;
    private String createdAt;
    private String statusApprove;
    private String dir;
    private String hrdApproveDate;
    private String updatedAt;
    private String executiv;
    private String selectCuti;
    private String headKyano;
    private String ket;
    private String kynm;
    private String ctjenisId;
    private String ctgambar;
    private String ctgantitgl;
    private String approveDirectur;
    private String kyano;
    private String dvano;
    private String jbano;
    private String ctmulai;
    private String executivKyano;
    private String executivApproveDate;
    private String ctjenis;
    private String approveHead;
    private String cttelp;
    private String headApproveDate;
    private String directurKyano;
    private String approveExecutiv;
    private String hrdKyano;
    private String ctalasan;
    private String ctano;
    private String ctselesai;
    private String ctperiode;
    private String directurApproveDate;
    private String headName;
    private String comment;
    private String ctlama;
    private String status;

    public void setCtgantihari(String ctgantihari){
        this.ctgantihari = ctgantihari;
    }

    public String getCtgantihari(){
        return ctgantihari;
    }

    public void setApproveHrd(String approveHrd){
        this.approveHrd = approveHrd;
    }

    public String getApproveHrd(){
        return approveHrd;
    }

    public void setHrdName(String hrdName){
        this.hrdName = hrdName;
    }

    public String getHrdName(){
        return hrdName;
    }

    public void setNmcuti(String nmcuti){
        this.nmcuti = nmcuti;
    }

    public String getNmcuti(){
        return nmcuti;
    }

    public void setDvnama(String dvnama){
        this.dvnama = dvnama;
    }

    public String getDvnama(){
        return dvnama;
    }

    public void setCtsisahak(String ctsisahak){
        this.ctsisahak = ctsisahak;
    }

    public String getCtsisahak(){
        return ctsisahak;
    }

    public void setCreatedAt(String createdAt){
        this.createdAt = createdAt;
    }

    public String getCreatedAt(){
        return createdAt;
    }

    public void setStatusApprove(String statusApprove){
        this.statusApprove = statusApprove;
    }

    public String getStatusApprove(){
        return statusApprove;
    }

    public void setDir(String dir){
        this.dir = dir;
    }

    public String getDir(){
        return dir;
    }

    public void setHrdApproveDate(String hrdApproveDate){
        this.hrdApproveDate = hrdApproveDate;
    }

    public String getHrdApproveDate(){
        return hrdApproveDate;
    }

    public void setUpdatedAt(String updatedAt){
        this.updatedAt = updatedAt;
    }

    public String getUpdatedAt(){
        return updatedAt;
    }

    public void setExecutiv(String executiv){
        this.executiv = executiv;
    }

    public String getExecutiv(){
        return executiv;
    }

    public void setSelectCuti(String selectCuti){
        this.selectCuti = selectCuti;
    }

    public String getSelectCuti(){
        return selectCuti;
    }

    public void setHeadKyano(String headKyano){
        this.headKyano = headKyano;
    }

    public String getHeadKyano(){
        return headKyano;
    }

    public void setKet(String ket){
        this.ket = ket;
    }

    public String getKet(){
        return ket;
    }

    public void setKynm(String kynm){
        this.kynm = kynm;
    }

    public String getKynm(){
        return kynm;
    }

    public void setCtjenisId(String ctjenisId){
        this.ctjenisId = ctjenisId;
    }

    public String getCtjenisId(){
        return ctjenisId;
    }

    public void setCtgambar(String ctgambar){
        this.ctgambar = ctgambar;
    }

    public String getCtgambar(){
        return ctgambar;
    }

    public void setCtgantitgl(String ctgantitgl){
        this.ctgantitgl = ctgantitgl;
    }

    public String getCtgantitgl(){
        return ctgantitgl;
    }

    public void setApproveDirectur(String approveDirectur){
        this.approveDirectur = approveDirectur;
    }

    public String getApproveDirectur(){
        return approveDirectur;
    }

    public void setKyano(String kyano){
        this.kyano = kyano;
    }

    public String getKyano(){
        return kyano;
    }

    public void setDvano(String dvano){
        this.dvano = dvano;
    }

    public String getDvano(){
        return dvano;
    }

    public void setJbano(String jbano){
        this.jbano = jbano;
    }

    public String getJbano(){
        return jbano;
    }

    public void setCtmulai(String ctmulai){
        this.ctmulai = ctmulai;
    }

    public String getCtmulai(){
        return ctmulai;
    }

    public void setExecutivKyano(String executivKyano){
        this.executivKyano = executivKyano;
    }

    public String getExecutivKyano(){
        return executivKyano;
    }

    public void setExecutivApproveDate(String executivApproveDate){
        this.executivApproveDate = executivApproveDate;
    }

    public String getExecutivApproveDate(){
        return executivApproveDate;
    }

    public void setCtjenis(String ctjenis){
        this.ctjenis = ctjenis;
    }

    public String getCtjenis(){
        return ctjenis;
    }

    public void setApproveHead(String approveHead){
        this.approveHead = approveHead;
    }

    public String getApproveHead(){
        return approveHead;
    }

    public void setCttelp(String cttelp){
        this.cttelp = cttelp;
    }

    public String getCttelp(){
        return cttelp;
    }

    public void setHeadApproveDate(String headApproveDate){
        this.headApproveDate = headApproveDate;
    }

    public String getHeadApproveDate(){
        return headApproveDate;
    }

    public void setDirecturKyano(String directurKyano){
        this.directurKyano = directurKyano;
    }

    public String getDirecturKyano(){
        return directurKyano;
    }

    public void setApproveExecutiv(String approveExecutiv){
        this.approveExecutiv = approveExecutiv;
    }

    public String getApproveExecutiv(){
        return approveExecutiv;
    }

    public void setHrdKyano(String hrdKyano){
        this.hrdKyano = hrdKyano;
    }

    public String getHrdKyano(){
        return hrdKyano;
    }

    public void setCtalasan(String ctalasan){
        this.ctalasan = ctalasan;
    }

    public String getCtalasan(){
        return ctalasan;
    }

    public void setCtano(String ctano){
        this.ctano = ctano;
    }

    public String getCtano(){
        return ctano;
    }

    public void setCtselesai(String ctselesai){
        this.ctselesai = ctselesai;
    }

    public String getCtselesai(){
        return ctselesai;
    }

    public void setCtperiode(String ctperiode){
        this.ctperiode = ctperiode;
    }

    public String getCtperiode(){
        return ctperiode;
    }

    public void setDirecturApproveDate(String directurApproveDate){
        this.directurApproveDate = directurApproveDate;
    }

    public String getDirecturApproveDate(){
        return directurApproveDate;
    }

    public void setHeadName(String headName){
        this.headName = headName;
    }

    public String getHeadName(){
        return headName;
    }

    public void setComment(String comment){
        this.comment = comment;
    }

    public String getComment(){
        return comment;
    }

    public void setCtlama(String ctlama){
        this.ctlama = ctlama;
    }

    public String getCtlama(){
        return ctlama;
    }

    public void setStatus(String status){
        this.status = status;
    }

    public String getStatus(){
        return status;
    }
}
